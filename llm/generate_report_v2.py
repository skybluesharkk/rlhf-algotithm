...
# ───────────────────────────────────────────────────────────────
# 1) 사용할 Instruction-tuned 공개 모델 지정
#    - 실제 Hugging Face에 존재하는 모델 예시: mosaicml/mpt-7b-instruct
...
mock_data = [
    ...
    (
        ...
        [{"name": "searchT"}, {"type": "text", "name": "searchK", "value": ""}],
        {}
    ),
]
...
def main():
    ...
    # 3) 생성 파이프라인 구성 (프롬프트 제외, 샘플링 + top-p)
    nlp = pipeline(
        "text-generation",
        model=model,
        tokenizer=tokenizer,
