...
#이미 BC학습한 파라미터 불러오기
bcpt = torch.load("bc_student_weights.pth", map_location=device)

# 방금 불러온 파라미터를 학생 네트워크랑 정책망에 각각 복사해주기
student_net.load_state_dict(bcpt)
target_net.load_state_dict(bcpt); target_net.eval()
...
writer      = SummaryWriter(log_dir=f"runs/weak_0.03_40_cosin_improved_exp_{int(time.time())}")
...
for ep in range(1, NUM_EPISODES+1):
    ...
    if ep % 100 == 0:
        ...
        }, f"weak_bc0.03_40_cosin_student_checkpoint_ep{ep}.pt")
...
torch.save(final_save_dict, "weak_bc0.03_40_student_final_improved_cosin.pt")
