...
class CarRacingDiscrete(gym.Wrapper):
    """CarRacing-v2 환경을 이산 액션 공간으로 변환하고 관측을 흑백으로 변환합니다."""
...
# 리플레이 메모리
# -----------------------------
Transition = namedtuple('Transition', ('state', 'action', 'next_state', 'reward'),'time',0)
...
# TensorBoard writer
writer = SummaryWriter(log_dir="runs/carracing_dqn")
...
for i_episode in range(1, NUM_EPISODES + 1):
    ...
    for t in range(MAX_STEPS_PER_EPISODE):
        ...
        loss1 = optimize_model()
        loss2 = optimize_model()
        for loss_val in (loss1, loss2):
