...
class ReplayMemory(object):
    ...
    def push(self, *args, cnt=1):
        """
        트랜지션을 cnt만큼 반복해서 메모리에 저장
        args: Transition의 구성요소들
        cnt: 저장할 반복 횟수
        """
...
REPLAY_COUNT = 2      # 리플레이 버퍼에 저장할 반복 횟수
...
def train_with_teacher_feedback(student_net, teacher_net, env, device, num_episodes=NUM_EPISODES):
    ...
    for i_episode in range(1, num_episodes + 1):
        ...
        for t in range(MAX_STEPS_PER_EPISODE):
            ...
            # 새로운 Transition 형식으로 저장 (고정된 반복 횟수 사용)
            memory.push(state, student_action, next_state, reward_tensor, 
                       current_time, feedback, cnt=REPLAY_COUNT)
...
if __name__ == "__main__":
    ...
    # Teacher 모델 로드
    teacher_checkpoint = torch.load("ori_carracing_dqn_final_model.pt")
