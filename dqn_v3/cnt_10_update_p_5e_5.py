"""
student_dqn_replay_buffer_gpu.py

Student DQN 학습 스크립트 (Teacher-Student 피드백 + AMP 최적화 + TensorBoard)
CNN 구조와 환경 래퍼는 ori_carracing_dqn_final_model_5e-5.py(Teacher)와 완전히 동일하도록 유지하였습니다.
"""
...
class CarRacingDiscrete(gym.Wrapper):
    """
    Gym CarRacing-v3 환경을 래핑하여
      - 7개의 이산 액션(steer left, steer right, gas, brake, gas+left, gas+right, no-op)
      - 그레이스케일 84×84 전처리
    """
    ACTIONS = [
...
writer = SummaryWriter(log_dir=f"runs/experiment_correct_update_p_cnt10_5e-5_{current_time}/student_replay_buffer")
...
for i_episode in range(1, NUM_EPISODES + 1):
    ...
    if i_episode % 50 == 0:
        ...
        }, f"student_correct_update_p_cnt10_5e-5_checkpoint_ep{i_episode}.pt")
...
}, "final_student_model_correct_update_p_cnt10_5e-5.pt")
...
print("🎉 Student_update_p_10_5e-5 모델 및 학습 그래프 저장 완료!")
