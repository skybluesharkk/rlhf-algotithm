...
writer = SummaryWriter(log_dir=f"runs/experiment_{current_time}/teacher")

# 추가: 리워드와 평균 손실을 저장할 리스트
all_rewards = []
all_mean_losses = []
...
for i_episode in range(1, NUM_EPISODES + 1):
    ...
    if i_episode % 50 == 0:
        ...
        }, f"carracing_dqn_checkpoint_ep{i_episode}.pt")

print("Training complete")

# 최종 모델 저장
final_checkpoint = {
    'model': policy_net,  # 전체 모델 저장
    'optimizer_state_dict': optimizer.state_dict(),
    'memory': memory,
    'steps_done': steps_done,
    'episode': NUM_EPISODES
}
torch.save(final_checkpoint, "ori_carracing_dqn_final_model.pt")
