...
# --- Replay Buffer ---
# Transition에 state, action, next_state, reward, time, feedback 저장
Transition = namedtuple('Transition', ('state', 'action', 'next_state', 'reward', 'time', 'feedback'))
...
# 고유 로그 디렉토리 생성: 현재 날짜와 시간 기반 (실행할 때마다 다른 폴더에 저장)
run_id = time.strftime("%Y%m%d-%H%M%S")
log_dir = f"./runs/dqn_experiment_{run_id}"
...
for i_episode in range(num_episodes):
    ...
    for step in count():
        current_time = time.time()  # 현재 절대 시각 기록
        # 예시로 total_reward가 양수일 경우 피드백 1, 아니면 0 (원하는 조건으로 수정 가능)
        feedback_val = 1 if total_reward > 0 else 0
        ...
        memory.push(state, action_idx, next_state, reward_tensor, current_time, feedback_val)
        state = next_state

        # optimize_model()를 한 스텝마다 두 번 호출하여 업데이트 수행
